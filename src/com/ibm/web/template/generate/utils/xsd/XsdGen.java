package com.ibm.web.template.generate.utils.xsd;
...
public final class XsdGen {
	...
	private void processAttributes(final Element inElement,
			final Element outElement) {
...
//			final String nsName = (nsPrefix == null ? "" : nsPrefix + ":") + name;
			...
			Element attrElement = new Element(XSD_PREFIX + ":attribute",
					XSD_NS_URI);
			attrElement.addAttribute(new Attribute("name", name));
			attrElement.addAttribute(new Attribute("type", XSD_PREFIX
					+ TypeInferenceUtil.getTypeOfContent(value)));
	...
	private void recurseGen(Element parent, Element parentOutElement) {
		...
		Element complexType = new Element(XSD_PREFIX + ":complexType",
				XSD_NS_URI);
		...
		for (int i = 0; i < childs.size(); i++) {
			...
			if (!elementNamesProcessed.contains(nsName)) { // process an element
															// first time only
				if (e.getChildElements().size() > 0) { // Is complex type with
														// children!
					Element element = new Element(XSD_PREFIX + ":element",
							XSD_NS_URI);
					...
					if (parent.getChildElements(localName, nsURI).size() > 1) {
						element.addAttribute(new Attribute("maxOccurs",
								"unbounded"));
					} else {
					...
					final String type = XSD_PREFIX
							+ TypeInferenceUtil.getTypeOfContent(eValue);
					Element element = new Element(XSD_PREFIX + ":element",
							XSD_NS_URI);
					element.addAttribute(new Attribute("name", localName));
					if (parent.getChildElements(localName, nsURI).size() > 1) {
					...
					if (attrCount > 0) { // has attributes: complex type without
											// sequence!
						Element complexTypeCurrent = new Element(XSD_PREFIX
								+ ":complexType", XSD_NS_URI);
						Element simpleContent = new Element(XSD_PREFIX
								+ ":simpleContent", XSD_NS_URI);
						Element extension = new Element(XSD_PREFIX
								+ ":extension", XSD_NS_URI);
	...
	private Document getDocument(File file) throws ParsingException,
			IOException {
		...
		// Setting targetNamespace:
		{
			...
			if (StringUtil.isNotEmpty(nsPrefix)) {
				outRoot.addAttribute(new Attribute("targetNamespace",
						rootElement.getNamespaceURI()));
				outRoot.addAttribute(new Attribute("elementFormDefault",
						"qualified"));
			}
		}
		...
		Element rootElementXsd = new Element(XSD_PREFIX + ":element",
				XSD_NS_URI);
		{
			rootElementXsd.addAttribute(new Attribute("name", rootElement
					.getLocalName()));
	...
	public void write(final OutputStream os) throws IOException {
		if (doc == null)
			throw new IllegalStateException(
					"Call parse() before calling this method!");
		write(os, Charsets.UTF_8);
	}

	public void write(final OutputStream os, final Charset charset)
			throws IOException {
		if (doc == null)
			throw new IllegalStateException(
					"Call parse() before calling this method!");
